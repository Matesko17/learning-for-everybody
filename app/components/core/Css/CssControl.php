...
class CssControl extends Control
{
    ...
    public function __construct($wwwDir, array $data, $environment, Translator $translator)
    {
        ...
        $this->templatePath = __DIR__ . '/css.latte';
    }
    ...
    public function render($source)
    {
        if (!array_key_exists($source, $this->data)) {
            throw new InvalidArgumentException('$source "' . $source . '" ' . $this->translator->translate('components.cssControl.notExists') . '.');
        }

        if (!array_key_exists('files', $this->data[$source])) {
            throw new InvalidArgumentException($this->data[$source] . ' - ' . $this->translator->translate('components.cssControl.missingFilesConfig') . '.');
        }

        if (!is_array($this->data[$source]['files'])) {
            throw new InvalidArgumentException($this->data[$source] . ' - ' . $this->translator->translate('components.cssControl.filesIsNotArray') . '.');
        }
